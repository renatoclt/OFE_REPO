...
var controladorSincronizacionPercepcion = function (ruta, rutaEsp) {
    router.get(ruta.concat('/'), async function (req, res) {
        try{
            var data = await comprobantePagoDTO.sincornizarPercepcion();
    ...
    router.post(ruta.concat('/actualizarFecha'), async function (req, res){
        try{
            await Sincronizacion.actualizarFecha(constantes.FILECMD.tipos_documento.percepcion, req.body.fecha);
            res.json({});
        }
        catch(e){
            res.json({ 'error': e})
